...
# 8-bit quantization on transformer only
pipeline_quant_config = PipelineQuantizationConfig(
    quant_backend="bitsandbytes_8bit",
    quant_kwargs={"load_in_8bit": True},
    components_to_quantize=["transformer"],  # Keep text encoders full precision
)
...
for idx, image in enumerate(images_):
    ...
    filename = f"{output_dir}/brian_{timestamp}_{idx}.png"
    image.save(filename)
